...
#define DEFAULT_IMAGE_LIST_SIZE 16
struct ImageList loadedImages = 
	{
		.images = NULL,
		._imageCount = 0,
		._maxImageCount = 0
...
void appendImageId(const char *path, unsigned int id)
{
	...
	{
		loadedImages._imageCount = 0;
		loadedImages._maxImageCount = DEFAULT_IMAGE_LIST_SIZE;
		loadedImages.images = (struct Image*)malloc(sizeof(struct Image) * DEFAULT_IMAGE_LIST_SIZE);
		
		loadedImages.images[loadedImages._imageCount].id = id;
		strncpy(loadedImages.images[loadedImages._imageCount].path, path, 255);
		loadedImages.images[loadedImages._imageCount].path[255] = '\0';
		loadedImages._imageCount++;
		return;
	}

	if(loadedImages._imageCount < loadedImages._maxImageCount)
	{
		loadedImages.images[loadedImages._imageCount].id = id;
		strncpy(loadedImages.images[loadedImages._imageCount].path, path, 255);
		loadedImages.images[loadedImages._imageCount].path[255] = '\0';	
	...
	{
		loadedImages._maxImageCount *= 2;
		loadedImages.images = realloc(loadedImages.images, sizeof(struct Image) * loadedImages._maxImageCount);
		loadedImages.images[loadedImages._imageCount].id = id;
		strncpy(loadedImages.images[loadedImages._imageCount].path, path, 255);
		loadedImages.images[loadedImages._imageCount].path[255] = '\0';	
		loadedImages._imageCount++;
